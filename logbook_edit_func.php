...
function art_form_updatedata_display($form, $message){
    ...
    art_sitemenu_display("logbook_edit", "logbook_edit_th");
    print "</td>\n";
    ...
    $url = "logbook_edit_ajax.php?rv=1";
    $ismaster = false;
            print "<tr>\n";
...
function art_update_data(){
    ...
    if (($result == "") && (art_session('art_form_loaded', 0) != 1)){
        ...
        $sql .= " logbook.id = " .  art_quote_intval($artv_id); 
        $sql .= ",logbook.no_rm = " .  art_quote_strval($artv_no_rm); 
        $sql .= ",logbook.tgl = " .  art_quote_dateval($artv_tgl); 
        $sql .= ",logbook.id_pel = " .  art_quote_intval($artv_id_pel); 
        $sql .= ",logbook.ket = " .  art_quote_strval($artv_ket); 
        $sql .= ",logbook.uid = " .  art_quote_strval($artv_uid); 
        $sql .= " WHERE ";
        $sql .= " logbook.id = " .  art_quote_intval($artv_pm_id);
    }
